...
# Config Imports (EFFECT_ID removed)
from config import BANNED_USERS, OWNER_ID, START_IMG_URL, CMBOT
...
def make_gp_panel(bot_username, txt_add, txt_support, support_chat):
    ...
    buttons = [
        [
            ...
            InlineKeyboardButton(text=txt_support, url=support_chat, style=group_style),
        ]
    ]
    return InlineKeyboardMarkup(buttons)
...
async def start_pm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if arg.startswith("help"):
            alone_style = random.choice(STYLES)
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(_["S_B_9"], url=C_SUPPORT_CHAT, style=alone_style)]])
        ...
        if arg.startswith("inf"):
            ...
            try:
                ...
                key = InlineKeyboardMarkup([[InlineKeyboardButton(_["S_B_8"], url=result["link"], style=group_style), InlineKeyboardButton(_["S_B_9"], url=C_SUPPORT_CHAT, style=group_style)]])
                await m.delete()
...
async def reset_start_settings(client, message):
    bot_id = (await client.get_me()).id
    # Added start_effect to $unset to ensure old data is cleaned up
